...
for ns1 in "${nslist[@]}"; do
	...
	for ns2 in "${nslist[@]}" ; do
        ...
        then
            ...
            # Warm-up ping to trigger ARP resolution and path convergence before flood test
            for h1 in "${!hosts1[@]}"
            do
                sudo ip netns exec $ns1 ping ${hosts2[h1]} -c 5 -W 10 2>&1> /dev/null
            done
            for size in ${sizes[@]}
            do
		        for h1 in "${!hosts1[@]}"
            ...
            for h2 in "${hosts2[@]}"
            do
                sudo ip netns exec $ns1 ping $h2 -c 5 -W 10 2>&1> /dev/null
            done
            for size in ${sizes[@]}
            do
		        for h1 in "${hosts1[@]}"
