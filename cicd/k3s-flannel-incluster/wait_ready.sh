...
function wait_cluster_ready {
    ...
    # to read and the cluster was reported ready.
    if ! output=$(sudo kubectl get nodes --no-headers 2>&1); then
    ...
    if ! output=$(sudo kubectl get pods -A --no-headers 2>&1); then
...
function wait_cluster_ready_full {
  ...
  for ((;;)) do
    ...
    if [[ $i -ge 40 ]]; then
        ...
        # provisioning log to work from.
        echo "--- what was still failing ---"
        echo "$LastErr"
        echo "--- nodes ---"
        sudo kubectl get nodes -o wide 2>&1
        echo "--- pods ---"
        sudo kubectl get pods -A -o wide 2>&1
        exit 1
    fi
