...
function health() {
    if [ "$1" == "strict" ]; then
        opt="-strict"
        echo "HTTPS strict probing" >&2
    else
        echo "HTTPS probing" >&2
    fi

    $hexec l3ep1 ./server/server -host server1 -key 31.31.31.1/key.pem -cert 31.31.31.1/cert.pem -cacert minica.pem $opt &
    ...
    if [ "$1" != "strict" ]; then
        if grep -q "VERSION_ID=\"24" /etc/os-release 2>/dev/null; then
            # On Ubuntu 24, the first VIP request can advance the RR pointer by one slot.
            # If it lands on server1, send two more requests so the main check starts from server1 again.
...
if [[ $code == 0 ]]
then
    echo SCENARIO-httpsep p1 [OK]
else
    echo SCENARIO-httpsep p1 [FAILED]
    exit $code
fi
...
if [[ $code == 0 ]]
then
    echo SCENARIO-httpsep p2 [OK]
else
    echo SCENARIO-httpsep p2 [FAILED]
    exit $code
fi
