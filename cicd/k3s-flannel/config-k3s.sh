...
apt-get update
apt-get install -y docker-ce curl wget iperf bridge-utils net-tools

sudo systemctl stop unattended-upgrades
sudo apt-get purge -y unattended-upgrades

git clone --recurse-submodule https://github.com/loxilb-io/loxilb.git
cd loxilb/cicd/k3s-flannel
...
if [[ -f "/usr/local/bin/k3s-uninstall.sh" ]]; then
  ...
  curl -sfL https://get.k3s.io | INSTALL_K3S_EXEC="server --disable traefik --disable servicelb --disable-cloud-controller --kubelet-arg cloud-provider=external --node-ip=192.168.80.9 --flannel-iface=eth1" K3S_KUBECONFIG_MODE="644" sh -
