...
docker-run:
	...
	docker run -u root --cap-add SYS_ADMIN   --restart unless-stopped --privileged -dt --entrypoint /bin/bash  --name $(dock) ghcr.io/loxilb-io/loxilb:$(TAG)

docker-rp: docker-run docker-cp
	@docker exec -it $(dock) mkllb_bpffs 2>&1 >> /dev/null || true
	docker commit ${loxilbid} ghcr.io/loxilb-io/loxilb:$(TAG)
	@docker stop $(dock) 2>&1 >> /dev/null || true
	@docker rm $(dock) 2>&1 >> /dev/null || true
...
docker:
	docker build -t ghcr.io/loxilb-io/loxilb:$(TAG) .

docker-arm64:
	docker  buildx build --platform linux/arm64 -t ghcr.io/loxilb-io/loxilb:$(TAG)-arm64 .
